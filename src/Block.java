...
public class Block {
    ...
    // Dynamic method for 100%
    public synchronized String computeMerkleRoot(ArrayList<String> lstItems) {

        ArrayList<MerkleNode> currentList = new ArrayList<>();

        for (int i = 0; i < lstItems.size(); i++) {
            MerkleNode currentNode = new MerkleNode();
            currentNode.sHash = BlockchainUtil.generateHash(lstItems.get(i));
            currentList.add(currentNode);
        }

        while (currentList.size() > 1) {
            ArrayList<MerkleNode> nextList = new ArrayList<>();
            for (int i = 0; i < currentList.size(); i += 2) {
                MerkleNode currentNode = new MerkleNode();
                populateMerkleNode(currentNode,
                        currentList.get(i), currentList.get(i + 1)
                );
                nextList.add(currentNode);
            }
            currentList = nextList;
        }
        return currentList.get(0).sHash;
    }
    ...
    private void populateMerkleNode(MerkleNode oNode, MerkleNode oLeftNode, MerkleNode oRightNode){
        BlockchainUtil bcu = new BlockchainUtil();
    ...
    public static void main(String[] args){
        ...
        lstItems.add("t8");
        sMerkleRoot = oBlock.computeMerkleRoot(lstItems);
        System.out.println("Merkle root method for 8 items output: ");
