...
public class Miner implements Runnable{
    ...
    public boolean doProofOfWork(Block oBlock) {
        ...
        // Using for loop instead of while loop out of preference
        for(int x = 0; x < oBlock.getDifficulty(); x++) {
                placeHolderHash += "0";
        }

        boolean nonceIsFound = false;
        int nonce = 0;
        ...
        while(true) {
            ...
            if(bAbortPoW) {
                ...
                String computedHash = oBlock.computeHash();
                oBlock.setHash(computedHash);

                if (oBlock.getHash().startsWith(placeHolderHash)) {
                    return true;
                }
            }
            nonce++;
